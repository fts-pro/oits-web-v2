...
export const metadata: Metadata = {
  title: 'Modernise & Decouple Legacy Systems',
  description: 'Legacy software modernisation, monolithic decoupling, database optimization, and zero-downtime cloud migration without disrupting live users.',
};

export default function ModernisePage() {
  const service = SERVICES_OUTCOMES.modernise;
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 max-w-5xl py-12 sm:py-16 space-y-16 text-left">
      ...
      {/* Explicit Scope Boundaries & Exclusions */}
      <div className="p-8 rounded-3xl bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 space-y-4">
        <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400 font-mono text-xs font-bold uppercase">
          ...
          <span>Explicit Scope Boundaries & Non-Goals</span>
        </div>
        <p className="text-xs text-slate-600 dark:text-slate-300">
          To protect client outcomes and prevent delivery failure, we enforce explicit boundaries:
        </p>
      ...
      {/* Governed Evidence Proof */}
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-slate-950 dark:text-white">
          Audited Benchmarks & Evidence
        </h2>
      ...
      <div className="p-8 rounded-3xl bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 space-y-6">
        <h2 className="text-xl font-bold text-slate-950 dark:text-white">
        ...
        <div className="space-y-4">
          {service.objections.map((obj, i) => (
            <div key={i} className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-2">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed pl-5.5">
                {obj.answer}
              </p>
            </div>
          ))}
      ...
      <div className="p-8 sm:p-10 rounded-3xl bg-white dark:bg-slate-900 text-slate-950 dark:text-white border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
        <div className="space-y-1">
          <h3 className="text-lg font-bold text-slate-950 dark:text-white">Have a legacy system needing modernisation?</h3>
          <p className="text-xs text-slate-600 dark:text-slate-300">Book a 90-minute Delivery Review with our Lead Systems Architect.</p>
        </div>
        <Link
